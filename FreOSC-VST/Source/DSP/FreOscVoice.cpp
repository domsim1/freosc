...
void FreOscVoice::renderNextBlock(juce::AudioBuffer<float>& outputBuffer, int startSample, int numSamples)
{
    ...
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        // Get FM modulation signal if active using dedicated FM oscillator
        float fmSignal = 0.0f;
        if (params.fmAmount > 0.0f && params.fmSource > 0) // fmSource > 0 means not "None"
        {
...
{
    // Generate FM modulation using dedicated FM oscillator
    // This avoids double-processing the source oscillators
    
    if (params.fmAmount <= 0.0f || params.fmSource == 0) // fmSource == 0 means "None"
        return 0.0f;
    
    // Get the frequency of the source oscillator for the FM modulator
    float sourceFreq = 0.0f;
    
    switch (params.fmSource.load())
    {
        case 1: // Use Osc1 as FM source reference (parameter index 1 = "Oscillator 1")
            sourceFreq = oscillator1.getCurrentFrequency();
            break;
        case 2: // Use Osc2 as FM source reference (parameter index 2 = "Oscillator 2")
            sourceFreq = oscillator2.getCurrentFrequency();
            break;
        case 3: // Use Osc3 as FM source reference (parameter index 3 = "Oscillator 3")
            sourceFreq = oscillator3.getCurrentFrequency();
            break;
        case 0: // None (parameter index 0)
        default:
            return 0.0f; // No valid FM source
    }
...
bool FreOscVoice::shouldReceiveFM(int oscillatorIndex)
{
    ...
    // Note: fmTarget parameter is 0-based (0=Osc1, 1=Osc2, 2=Osc3, 3=All)
    ...
    switch (fmTarget)
    {
        ...
        case 2: // Osc3 only
            return oscillatorIndex == 3;
        case 3: // All oscillators
            return true;
        default:
            return false;
    }
